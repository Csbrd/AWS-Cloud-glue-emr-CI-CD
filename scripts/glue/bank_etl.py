...
job         = Job(glueContext)
job.init(args["JOB_NAME"], args)  # Bookmark 활성화
...
raw_df = glueContext.create_dynamic_frame.from_options(
    ...
    format="json",
    transformation_ctx=f"{SOURCE}_raw_src",  # Bookmark 추적 키
).toDF()
...
normalized_df = (
    filtered_df
    .withColumn("bank_id",            F.col("bank_id").cast(StringType()))
    .withColumn("global_customer_id", F.col("global_customer_id").cast(StringType()))
    .withColumn("balance",            F.col("balance").cast(LongType()))
    .withColumn("transaction_amount", F.col("transaction_amount").cast(LongType()))
